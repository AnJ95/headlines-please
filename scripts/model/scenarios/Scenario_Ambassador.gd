...
func init_messages():
    messages.append(Message.new(Message.TWITTER, [1,2], "I heard gun shots near %ss embassy. Did something happen?%s"))
    ...
    messages.append(Message.new(Message.LIVE, [1,3], "The autopsy finds toxic substances in the Ambassadors body%s%s"))
    messages.append(Message.new(Message.TWITTER, [0,2], "Hallo Welt%s%s"))
